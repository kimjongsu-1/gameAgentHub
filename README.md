...
대시보드의 `캐릭터 통일성 테스트 업로드`에서 일러스트 1장을 올리면 허브가 다음 작업을 자동으로 준비합니다.

- 참조 이미지를 `03_character_masters/uploads`에 저장
- `게임 개발 디자인`용 `character_consistency_test` 작업 생성
- 얼굴, 체형, 실루엣, 의상, 색상, 무기/소품 통일성 체크 조건 생성
- dispatch 큐 생성
